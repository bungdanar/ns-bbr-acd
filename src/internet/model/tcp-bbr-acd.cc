...
namespace ns3
{
  ...
  {
    static TypeId tid = TypeId("ns3::TcpBbrAcd")
                            ...
                            .AddAttribute("HighGain",
                                          "Value of high gain",
                                          DoubleValue(2.89),
                                          MakeDoubleAccessor(&TcpBbrAcd::m_highGain),
                            ...
                            .AddAttribute("RttWindowLength",
                                          ...
                                          MakeTimeAccessor(&TcpBbrAcd::m_rtPropFilterLen),
                                          MakeTimeChecker())
                            .AddAttribute("ProbeRttDuration",
                                          "Length of bandwidth windowed filter",
                                          TimeValue(MilliSeconds(200)),
                                          MakeTimeAccessor(&TcpBbrAcd::m_probeRttDuration),
                                          MakeTimeChecker());
    return tid;
  }
  ...
  TcpBbrAcd::PrintToFile(Ptr<const TcpSocketState> tcb)
  {
    ...
    if (!startPrintToFile)
    {
      ...
      std::ofstream traceAll("TcpBbrAcd_trace_for_node_" +
                                 std::to_string(node->GetId()) + ".data",
  ...
  TcpBbrAcd::ProbeCongestion(Ptr<TcpSocketState> tcb)
  {
    ...
    {
      if (tcb->m_lastRtt.Get().GetMicroSeconds() >= savedLastRtt - 1000 && tcb->m_lastRtt.Get().GetMicroSeconds() <= savedLastRtt + 1000)
      {
        countIfSteadyRtt++;
      }
      else
    ...
    if (countIfSteadyRtt >= 2 || tcb->m_lastRtt.Get().GetMicroSeconds() > 2 * tcb->m_minRtt.GetMicroSeconds())
    {
  ...
  TcpBbrAcd::LimitCwndToStopLoss(Ptr<TcpSocketState> tcb)
  {
    if (probeCongestion || tcb->m_congState == TcpSocketState::CA_RECOVERY)
    {
      ...
      std::cout << "\n\n\n\n\n\n\n"
                << Simulator::Now().GetSeconds() << ": node: " << this->GetNode()->GetId() << ": isCongestion - Congestion limit applied\n\n\n\n\n";
    }
    else
  ...
  void
  TcpBbrAcd::CwndEvent(Ptr<TcpSocketState> tcb,
